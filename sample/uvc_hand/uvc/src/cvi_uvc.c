...
int32_t run_ai(VIDEO_FRAME_INFO_S *venc_frame) {
	...
	//printf("---------------------openmodel-----------------------");
	//手部
	s32Ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_HAND_DETECTION, "/mnt/data/nfs/model/hand_det_qat_640x384.cvimodel");
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_OpenModel CVI_TDL_SUPPORTED_MODEL_HAND_DETECTION failed with %#x!\n", s32Ret);
	}
  	CVI_TDL_SetModelThreshold(tdl_handle, CVI_TDL_SUPPORTED_MODEL_HAND_DETECTION, 0.5);	

	//手部关键点
	s32Ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_HAND_KEYPOINT, "/mnt/data/nfs/model/hand_kpt_128x128.cvimodel");
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_OpenModel CVI_TDL_SUPPORTED_MODEL_HAND_KEYPOINT failed with %#x!\n", s32Ret);
	}

	//手部关键点分类
	s32Ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_HAND_KEYPOINT_CLASSIFICATION, "/mnt/data/nfs/model/hand_kpt_cls9.cvimodel");
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_OpenModel CVI_TDL_SUPPORTED_MODEL_HAND_KEYPOINT_CLASSIFICATION failed with %#x!\n", s32Ret);
	}
	...
	//手识别
	s32Ret = CVI_TDL_Detection(tdl_handle, venc_frame, CVI_TDL_SUPPORTED_MODEL_HAND_DETECTION, &obj_meta);
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_Detection CVI_TDL_SUPPORTED_MODEL_HAND_DETECTION failed with %#x!\n", s32Ret);
	}

	//手关键点识别
	if (obj_meta.size>0) {
		hand_obj.size = obj_meta.size;
		hand_obj.info =(cvtdl_handpose21_meta_t *)malloc(sizeof(cvtdl_handpose21_meta_t) * hand_obj.size);
		hand_obj.height = venc_frame->stVFrame.u32Height;
		hand_obj.width = venc_frame->stVFrame.u32Width;
		...
		CVI_TDL_HandKeypoint(tdl_handle, venc_frame, &hand_obj);
	}

	//手关键点分类
	for (uint32_t i = 0; i < hand_obj.size; i++) {
		//每一只手
		int num_keypoints = 42; // 假设有 21 个关键点的 x 和 y
		float *keypoints = (float *)malloc(num_keypoints * sizeof(float));
		for (uint32_t j = 0; j < 21; j++) {
			keypoints[j * 2] = hand_obj.info[i].xn[j]; // x 关键点
        	keypoints[j * 2 + 1] = hand_obj.info[i].yn[j]; // y 关键点
		}
		...
		if (num_keypoints != 42) {
			...
			CVI_U8 buffer[num_keypoints * sizeof(float)];
			memcpy(buffer, &keypoints[0], sizeof(float) * num_keypoints);
			VIDEO_FRAME_INFO_S Frame;
			Frame.stVFrame.pu8VirAddr[0] = buffer;
			Frame.stVFrame.u32Height = 1;
			Frame.stVFrame.u32Width = num_keypoints;
			s32Ret = CVI_TDL_HandKeypointClassification(tdl_handle, &Frame, &meta);
			if (s32Ret != CVI_SUCCESS) {
				printf("CVI_TDL_Detection CVI_TDL_HandKeypointClassification failed with %#x!\n", s32Ret);
			} else {
				char info[256];
				int label = meta.label;
				if (label>8 || label<0) {
					label = 9;
				}
				sprintf(info, "%s", gestures[label]);
				CVI_TDL_Service_ObjectWriteText(info, hand_obj.info[i].bbox_x, hand_obj.info[i].bbox_y-5, venc_frame, 0, 0, 255);
			}
		}
	...
	//关键点
	if (obj_meta.size>0) {
		CVI_TDL_Service_ObjectDrawRect(tdl_handle, &obj_meta, venc_frame, false, brushi);

		CVI_TDL_Service_DrawHandKeypoint(tdl_handle, venc_frame, &hand_obj);
	}

    
	gettimeofday(&stop_time, NULL);

    double elapsed = (__get_us(stop_time) - __get_us(start_time))  / 1000;

	if (elapsed>0) {
		char info[128];
		sprintf(info, "Cost:%0.1f", elapsed);
		CVI_TDL_Service_ObjectWriteText(info, 30, 50, venc_frame, 0, 0, 255);
	}
