...
int32_t sys_init(void)
{
	...
	u32BlkSize = COMMON_GetPicBufferSize(s_stMwCtx.stSize.u32Width, s_stMwCtx.stSize.u32Height, PIXEL_FORMAT_RGB_888,
		DATA_BITWIDTH_8, COMPRESS_MODE_NONE, DEFAULT_ALIGN);
	stVbConf.astCommPool[stVbConf.u32MaxPoolCnt].u32BlkSize = u32BlkSize;
	stVbConf.astCommPool[stVbConf.u32MaxPoolCnt].u32BlkCnt = 16;
...
int32_t run_ai(VIDEO_FRAME_INFO_S *venc_frame){
	...
	//printf("---------------------openmodel-----------------------");
	//人脸侦测
	s32Ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_RETINAFACE, "/mnt/data/model/retinaface_mnet0.25_342_608.cvimodel");
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_OpenModel CVI_TDL_SUPPORTED_MODEL_RETINAFACE failed with %#x!\n", s32Ret);
	}

	//人脸关键点
	s32Ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_FACERECOGNITION, "/mnt/data/model/cviface-v5-s.cvimodel");
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_OpenModel CVI_TDL_SUPPORTED_MODEL_FACERECOGNITION failed with %#x!\n", s32Ret);
	}

	//闭眼
	s32Ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_EYECLASSIFICATION, "/mnt/data/model/eye_v1_bf16.cvimodel");
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_OpenModel CVI_TDL_SUPPORTED_MODEL_EYECLASSIFICATION failed with %#x!\n", s32Ret);
	}
	//打哈欠
	s32Ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_YAWNCLASSIFICATION, "/mnt/data/model/yawn_v1_bf16.cvimodel");
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_OpenModel CVI_TDL_SUPPORTED_MODEL_YAWNCLASSIFICATION failed with %#x!\n", s32Ret);
	}
	...
	//脸识别
	CVI_TDL_FaceDetection(tdl_handle, venc_frame, CVI_TDL_SUPPORTED_MODEL_RETINAFACE, &stFaceMeta);
	
	if (stFaceMeta.size>0) {
		//关键点
		s32Ret = CVI_TDL_FaceRecognition(tdl_handle, venc_frame, &stFaceMeta);
		if (s32Ret != CVI_SUCCESS) {
			printf("CVI_TDL_FaceRecognition CVI_TDL_SUPPORTED_MODEL_FACERECOGNITION failed with %#x!\n", s32Ret);
		}

		//闭眼
		s32Ret = CVI_TDL_EyeClassification(tdl_handle, venc_frame, &stFaceMeta);
		if (s32Ret != CVI_SUCCESS) {
			printf("CVI_TDL_EyeClassification failed with %#x!\n", s32Ret);
		} else {
			printf("CVI_TDL_EyeClassification ok \n");
			printf("score=%f \n", stFaceMeta.dms->leye_score);
		}

		//打哈欠
		s32Ret = CVI_TDL_YawnClassification(tdl_handle, venc_frame, &stFaceMeta);
		if (s32Ret != CVI_SUCCESS) {
			printf("CVI_TDL_YawnClassificationfailed with %#x!\n", s32Ret);
		} else {
			printf("CVI_TDL_EyeClassification ok \n");
			printf("score=%f \n", stFaceMeta.dms->yawn_score);
		}
	}
	...
	if (stFaceMeta.size>0) {
		CVI_TDL_Service_FaceDrawRect(tdl_handle, &stFaceMeta, venc_frame, false, brushi);
	}
