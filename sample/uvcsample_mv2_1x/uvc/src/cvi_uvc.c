...
int32_t run_ai(VIDEO_FRAME_INFO_S *venc_frame){
	...
	//printf("---------------------openmodel-----------------------");
	s32Ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_PERSON_VEHICLE_DETECTION, "/mnt/data/nfs/model/yolov8n_384_640_person_vehicle.cvimodel");
	CVI_TDL_SetModelThreshold(tdl_handle, CVI_TDL_SUPPORTED_MODEL_PERSON_VEHICLE_DETECTION, 0.25);
	if (s32Ret != CVI_SUCCESS) {
	...
	s32Ret = CVI_TDL_PersonVehicle_Detection(tdl_handle, venc_frame, &obj_meta);

	if (s32Ret != CVI_SUCCESS) {
		printf("##CVI_TDL_PersonVehicle_Detection CVI_TDL_SUPPORTED_MODEL_PERSON_VEHICLE_DETECTION failed with %#x! \n", s32Ret);
		//return -1;
	}
