...
cvitdl_handle_t g_tdl_spg_handle = NULL;
cvitdl_handle_t g_tdl_foreign_obj_handle = NULL;

cvitdl_service_handle_t g_service_spg_handle = NULL;
cvitdl_service_handle_t g_service_foreign_obj_handle = NULL;

static cvtdl_object_t g_spg_obj_meta = {0};
static cvtdl_object_t g_foreign_obj_meta = {0};

static volatile bool s_ai_init = false;
static volatile bool jobExit = false;

static volatile bool run_foreign_obj_detection = false;
static volatile bool run_spg_detection = false;
...
void *run_tdl_thread(void *args) {
	...
	while (jobExit == false) {
		...
		cvtdl_object_t spg_HandMeta = {0};
		cvtdl_object_t foreign_obj_HandMeta = {0};
		memset(&spg_HandMeta, 0, sizeof(cvtdl_object_t));
		memset(&foreign_obj_HandMeta, 0, sizeof(cvtdl_object_t));
		...
		if (count <= 5){
			sleep(1);
			run_foreign_obj_detection = false;
			run_spg_detection = false;
			count ++;
		} else if (5 < count && count <= 100){
			//算法 -检测异物
			s32Ret = CVI_TDL_Detection(g_tdl_foreign_obj_handle, &stFrame, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, &foreign_obj_HandMeta);
			if (s32Ret != CVI_SUCCESS) {
				printf("##run_tdl_thread CVI_TDL_Detection_Windows failed with %#x! \n", s32Ret);
				goto detection_error;
			}
			run_foreign_obj_detection = true;
			run_spg_detection = false;
			count ++;
			printf("tdl foreign_obj thread run -detection obj=%d\n", foreign_obj_HandMeta.size);
		} else if (100 < count && count <= 200){
			//算法 -检测炒面
			s32Ret = CVI_TDL_Detection(g_tdl_spg_handle, &stFrame, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, &spg_HandMeta);
			if (s32Ret != CVI_SUCCESS) {
				printf("##run_tdl_thread CVI_TDL_Detection_Windows failed with %#x! \n", s32Ret);
				goto detection_error;
			}
			run_foreign_obj_detection = false;
			run_spg_detection = true;
			count ++;
			printf("tdl spg thread run -detection spg=%d\n", spg_HandMeta.size);
		} else if (count > 200) {
			count = 0;
			run_foreign_obj_detection = false;
			run_spg_detection = false;
...

		{
      		...
      		CVI_TDL_CopyObjectMeta(&spg_HandMeta, &g_spg_obj_meta);
			CVI_TDL_CopyObjectMeta(&foreign_obj_HandMeta, &g_foreign_obj_meta);
    	}

		CVI_VPSS_ReleaseChnFrame(0, 1, &stFrame);
		CVI_TDL_Free(&spg_HandMeta);
		CVI_TDL_Free(&foreign_obj_HandMeta);
	...
	CVI_TDL_DestroyHandle(g_tdl_spg_handle);
	CVI_TDL_DestroyHandle(g_tdl_foreign_obj_handle);
	CVI_TDL_Service_DestroyHandle(g_service_spg_handle);
	CVI_TDL_Service_DestroyHandle(g_service_foreign_obj_handle);
	CVI_TDL_Free(&g_spg_obj_meta);
	CVI_TDL_Free(&g_foreign_obj_meta);

  	pthread_exit(NULL);
}

CVI_S32 init_algo_spg_param(void) {
	InputPreParam preprocess_cfg = CVI_TDL_GetPreParam(g_tdl_spg_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION);
	...
	CVI_S32 ret = CVI_TDL_SetPreParam(g_tdl_spg_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, preprocess_cfg);
	if (ret != CVI_SUCCESS) {
	...
	cvtdl_det_algo_param_t yolov8_param = CVI_TDL_GetDetectionAlgoParam(g_tdl_spg_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION);
	yolov8_param.cls = 1;

	//printf("setup yolov8 algorithm param \n");
	ret = CVI_TDL_SetDetectionAlgoParam(g_tdl_spg_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, yolov8_param);
	if (ret != CVI_SUCCESS) {
	...
	// set theshold
	CVI_TDL_SetModelThreshold(g_tdl_spg_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, 0.25);
	CVI_TDL_SetModelNmsThreshold(g_tdl_spg_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, 0.25);

	return ret;
}

CVI_S32 init_algo_foreign_obj_param(void) {
	InputPreParam preprocess_cfg = CVI_TDL_GetPreParam(g_tdl_foreign_obj_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION);
	...
	//printf("setup yolov8 param \n");
	CVI_S32 ret = CVI_TDL_SetPreParam(g_tdl_foreign_obj_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, preprocess_cfg);
	if (ret != CVI_SUCCESS) {
		printf("Can not set yolov8 preprocess parameters %#x\n", ret);
		return ret;
	}
	...
	// setup yolo algorithm preprocess
	cvtdl_det_algo_param_t yolov8_param = CVI_TDL_GetDetectionAlgoParam(g_tdl_foreign_obj_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION);
	yolov8_param.cls = 1;

	//printf("setup yolov8 algorithm param \n");
	ret = CVI_TDL_SetDetectionAlgoParam(g_tdl_foreign_obj_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, yolov8_param);
	if (ret != CVI_SUCCESS) {
		printf("Can not set yolov8 algorithm parameters %#x\n", ret);
		return ret;
	}
	...
	// set theshold
	CVI_TDL_SetModelThreshold(g_tdl_foreign_obj_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, 0.35);
	CVI_TDL_SetModelNmsThreshold(g_tdl_foreign_obj_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, 0.35);
...
int32_t ai_init(void)
{
	printf("111-ai_init\n");
	int32_t s32Ret = 0;
	s32Ret = CVI_TDL_CreateHandle(&g_tdl_spg_handle);
	if (s32Ret != CVI_SUCCESS) {
		printf("Create spg tdl handle failed with %#x!\n", s32Ret);
		return -1;
	}

	s32Ret = CVI_TDL_CreateHandle(&g_tdl_foreign_obj_handle);
	if (s32Ret != CVI_SUCCESS) {
		printf("Create foreign_obj tdl handle failed with %#x!\n", s32Ret);
		return -1;
	}

	s32Ret = CVI_TDL_Service_CreateHandle(&g_service_spg_handle, g_tdl_spg_handle);
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_Service_CreateHandle spg failed with %#x!\n", s32Ret);
		return -1;
	}

	s32Ret = CVI_TDL_Service_CreateHandle(&g_service_foreign_obj_handle, g_tdl_foreign_obj_handle);
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_TDL_Service_CreateHandle foreign obj failed with %#x!\n", s32Ret);
		return -1;
	}

	//printf("---------------------openmodel-----------------------");
	s32Ret = init_algo_spg_param();
	if (s32Ret != CVI_SUCCESS) {
	...
	s32Ret = CVI_TDL_OpenModel(g_tdl_spg_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, "/mnt/data/nfs/model/yolov8s_3d_spg_cv181x_int8.cvimodel");
	if (s32Ret != CVI_SUCCESS) {
		printf("open model failed with %#x!\n", s32Ret);
		return -1;
	}

	s32Ret = CVI_TDL_OpenModel(g_tdl_foreign_obj_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, "/mnt/data/nfs/model/yolov8s_3d_foreign_obj_cv181x_int8.cvimodel");
	if (s32Ret != CVI_SUCCESS) {
		printf("open model failed with %#x!\n", s32Ret);
		return -1;
	}

  	memset(&g_spg_obj_meta, 0, sizeof(cvtdl_object_t));
	memset(&g_foreign_obj_meta, 0, sizeof(cvtdl_object_t));
...
int32_t run_ai_draw(VIDEO_FRAME_INFO_S *venc_frame){
	...
	{
      MutexAutoLock(ResultMutex, lock);
      CVI_TDL_CopyObjectMeta(&g_foreign_obj_meta, &foreign_obj_HandMeta);
	  CVI_TDL_CopyObjectMeta(&g_spg_obj_meta, &spg_HandMeta);
    }
	...
	//异物
	s32Ret = CVI_TDL_Service_ObjectDrawRect(g_service_foreign_obj_handle, &foreign_obj_HandMeta, venc_frame, true, brushi1);
	if (s32Ret != CVI_SUCCESS) {
	...
	//炒面
	s32Ret = CVI_TDL_Service_ObjectDrawRect(g_service_spg_handle, &spg_HandMeta, venc_frame, true, brushi2);
	if (s32Ret != CVI_SUCCESS) {
		printf("##CVI_TDL_Service_ObjectDrawRect failed with %#x!\n", s32Ret);
		return s32Ret;
	}
	...
	//异物
	for (uint32_t i = 0; i < foreign_obj_HandMeta.size; i++) {
		// printf("---- %f %f %f %f %d %f \n", foreign_obj_HandMeta.info[i].bbox.x1,
		// 	foreign_obj_HandMeta.info[i].bbox.y1,
		// 	foreign_obj_HandMeta.info[i].bbox.x2,
		// 	foreign_obj_HandMeta.info[i].bbox.y2,
		// 	foreign_obj_HandMeta.info[i].classes,
		// 	foreign_obj_HandMeta.info[i].bbox.score);

		char strinfo[128];
		sprintf(strinfo, "%0.2f", foreign_obj_HandMeta.info[i].bbox.score);
		s32Ret = CVI_TDL_Service_ObjectWriteText(strinfo, foreign_obj_HandMeta.info[i].bbox.x1, foreign_obj_HandMeta.info[i].bbox.y2, venc_frame, 0, 200, 0);
		if (s32Ret != CVI_SUCCESS) {
	...
	//炒面
	for (uint32_t i = 0; i < spg_HandMeta.size; i++) {
		// printf("---- %f %f %f %f %d %f \n", spg_HandMeta.info[i].bbox.x1,
		// 	spg_HandMeta.info[i].bbox.y1,
		// 	spg_HandMeta.info[i].bbox.x2,
		// 	spg_HandMeta.info[i].bbox.y2,
		// 	spg_HandMeta.info[i].classes,
		// 	spg_HandMeta.info[i].bbox.score);

		char strinfo[128];
		sprintf(strinfo, "%0.2f", spg_HandMeta.info[i].bbox.score);
		s32Ret = CVI_TDL_Service_ObjectWriteText(strinfo, spg_HandMeta.info[i].bbox.x1, spg_HandMeta.info[i].bbox.y2, venc_frame, 0, 200, 0);
		if (s32Ret != CVI_SUCCESS) {
			printf("##CVI_TDL_Service_ObjectWriteText failed with %#x!\n", s32Ret);
			return s32Ret;
		}
	...
	char info[128];
	if (!run_foreign_obj_detection && !run_spg_detection) {
		sprintf(info, "Stop detection");
	} else if (run_foreign_obj_detection) {
		sprintf(info, "Foreign obj detection : %d", foreign_obj_HandMeta.size);
	} else if (run_spg_detection) {
		sprintf(info, "Spaghetti detection : %d", spg_HandMeta.size);
	}
